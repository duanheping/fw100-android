...
public final class GsmDataConnectionTracker extends DataConnectionTracker {
    protected final String LOG_TAG = "GSMDCT";
    ...
    // ENABLE DBG debug log
    // see log method at end of this file
    private boolean DBG = true;

    // SIMSIM stateful Apn default builder
    private boolean defaultApnIsReady = false;
    ...
    private boolean trySetupData(String reason) {
        ...
        if (DBG) log("trySetupData: disabled test for sim getRecordsLoaded");
     ...
     * dispatched from message handler 
     * on data setup complete or data state change
     */
    private void connectivityCheck(int event) {

	ConnectivityManager connectivity;
	NetworkInfo wifiInfo, mobileInfo;

	if ((event != EVENT_DATA_SETUP_COMPLETE)
	&& (event != EVENT_DATA_STATE_CHANGED))
	{
		return;
	}
	...
	{
		connectivity = 
		(ConnectivityManager) phone.getContext().getSystemService(Context.CONNECTIVITY_SERVICE);
		wifiInfo = connectivity.getNetworkInfo(ConnectivityManager.TYPE_WIFI);
		mobileInfo = connectivity.getNetworkInfo(ConnectivityManager.TYPE_MOBILE);	
        	log("connectivityCheck Wifi info: " + wifiInfo.toString());
        	log("connectivityCheck 3G   info: " + mobileInfo.toString());
	}
    }

    /*  
     * write a single-entry APN list from operator MCCMNC 
     * If operator is not implemented in this function, then 
     * makes a generic APN that will suffice for data connection 
     */
    private ArrayList<ApnSetting> createApnFromOperator(String operator) {
	...
	*  9. String password;
	* 10. int authType;
	* 11. public String[] types;
	* 12. int id;
	* 13. String numeric;
	* 14. String protocol;
	* 15. String roamingProtocol;
 	*
	* public ApnSetting(int id, String numeric, String carrier, String apn,
	...
	int compare;
	String myTypes = Phone.APN_TYPE_ALL + "," + Phone.APN_TYPE_DEFAULT;
        String[] types = parseTypes(myTypes);

	/* Verizon */
	compare = operator.compareTo("310004");
        if (compare == 0) 
	{
        	if (DBG) log("createApnFromOperator make Verizon for " + operator);
		...
		null, 		/* proxy */
		null, 		/* port */
		"http://mms.vtext.com/servlets/mms", /* mmsc */
		null, 		/* mmsProxy */
		null, 		/* mmsPort */
		"*", 		/* user */
		"*", 		/* password */
		...
		types, 		/* types */
		null, 		/* protocol */
		null 		/* roamingProtocol */
		);

    		defaultApnIsReady = true;
        	result.add(apn);
		return result;
	}

	/* Sprint */
	compare = operator.compareTo("310120");
        if (compare == 0) 
	{
        	if (DBG) log("createApnFromOperator make Sprint for " + operator);
        	ApnSetting apn = new ApnSetting(
		1,		/* id */
		"310120",	/* numeric */
		"Sprint",	/* carrier */
        ...
        if (DBG) log("createApnFromOperator make generic for " + operator);
    ...
    private GsmDataConnection findFreePdp() {
        if (DBG) log("findFreePdp searching pdpList");
        for (DataConnection conn : pdpList) {
            GsmDataConnection pdp = (GsmDataConnection) conn;
	    if (DBG) log("findFreePdp checking pdp.isInactive" + pdp);
            if (pdp.isInactive()) {
                return pdp;
            }
        }
     ...
     * Includes fallback if SIM operator is undefined, 
     * then get operator name from NetworkInfo.
     * If APN is undefined, then make a default APN for the operator.
     *
     */
    private void createAllApnList() {
        allApns = new ArrayList<ApnSetting>();
        String operator = mGsmPhone.mSIMRecords.getSIMOperatorNumeric();
 	String network = TelephonyManager.getDefault().getNetworkOperator();

        if (operator == null) {
            if (DBG) log("createAllApnList: null operator from SIM, using network instead");
            if (DBG) log("createAllApnList: TelephonyManager network is " + network);
            operator = network;
	}
        ...
        if (allApns.isEmpty()) {
            if (DBG) log("No APN found for carrier: " + operator + " making default");
            /* preferredApn = null; */
            /* notifyNoData(GsmDataConnection.FailCause.MISSING_UNKNOWN_APN); */
            /* make a default APN using operator */
    	    allApns = createApnFromOperator(operator);
       	} 
        preferredApn = getPreferredApn();
        Log.d(LOG_TAG, "Get PreferredAPN");
        if (preferredApn != null && !preferredApn.numeric.equals(operator)) {
            if (DBG) log("createAllApnList set preferredApn null");
            preferredApn = null;
            setPreferredApn(-1);
        }
	else 
	{
            if (DBG) log("createAllApnList set preferredApn " + preferredApn);
	}
    }
    ...
    private void createAllPdpList() {
        ...
        if (DBG) log("createAllPdpList pool size is " + PDP_CONNECTION_POOL_SIZE);
        for (int i = 0; i < PDP_CONNECTION_POOL_SIZE; i++) {
            pdp = GsmDataConnection.makeDataConnection(mGsmPhone);
            pdpList.add(pdp);
            if (DBG) log("createAllPdpList added #" + i + " pdp " + pdp);
         }
    }
    ...
    private ArrayList<ApnSetting> buildWaitingApns() {
        ...
        if (mRequestedApnType.equals(Phone.APN_TYPE_DUN)) {
            Log.d(LOG_TAG, "buildWaitingApns mRequestedApnType is DUN");
        ...
        if (DBG) log("buildWaitingApns state is: " +
                    " operator=" + operator +
                    " mRequestedApnType=" + mRequestedApnType +
                    " Phone.APN_TYPE_DEFAULT=" + Phone.APN_TYPE_DEFAULT +
        ...
        if (allApns != null) {
            if (DBG) log("buildWaitingApns process APN list");
            for (ApnSetting apn : allApns) {
                if (DBG) log("buildWaitingApns checking apn " + apn);
                if (apn.canHandleType(mRequestedApnType)) {
                    apnList.add(apn);
                }
                else {
                    if (DBG) log("buildWaitingApns warning no match type " + mRequestedApnType);
                    apnList.add(apn);
		}
            }
        }
	...
	{
            if (DBG) log("buildWaitingApns allAPns is NULL");
	}
    ...
    public void handleMessage (Message msg) {
		...
		/* FIXME force one-time APN build in case EVENT_RECORDS_LOADED */
		/* did not preceed this event.  used for SIMSIM GSM emulation.  */
        	if (DBG) log("handleMessage: force APN rebuild");
    		if (defaultApnIsReady == false)
		{
                	onRecordsLoaded();
		}
                onGprsAttached();
        	...
        	if (DBG) log("handleMessage: disabled APN changed dispatch");
		/* disable this because data call is torn down on apn change */
		/* and creates a deferred tear-down that clobbers the data call */
                /* onApnChanged(); */
                break;
	...
	/* check status of wifi and 3g connections */
    	connectivityCheck(msg.what);
    }
